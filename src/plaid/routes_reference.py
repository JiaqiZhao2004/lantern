"""
Reference / demo endpoints that rely on a global in-memory access_token.
These are NOT wired into the live application router.  Keep them here as
implementation references while building out the production equivalents.
"""
...
# ====== GLOBAL STATE ======
# NOTE: This is demo-only. In production, access tokens must be fetched per-user
# from the database (see routes.py → POST /item).
...
# ---------- Reference router (not registered in server.py) ----------
router = APIRouter(prefix="/api/v1/plaid/ref", tags=["plaid-reference"])
