...
This component handles first-time backend ingress bring-up for `lantern-api.royzhao.dev`. The hostname starts as a proof endpoint in Slice 3 and is promoted to the protected CloudFront backend origin in Slice 4.
...
The ingress hostname must stay proxied through Cloudflare. `DNS only` is not sufficient here because there is no public backend listener to connect to directly. The host-local `nginx` process stays bound to loopback and is reachable only through the tunnel path.
...
3. Create the proxied DNS route for the ingress hostname:
...
9. Validate the public proof hostname before Slice 4 origin protection is enabled:
