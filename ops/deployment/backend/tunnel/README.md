...
This directory owns locally managed Cloudflare Tunnel enrollment, config rendering, and tunnel validation for `lantern-api.royzhao.dev` and `lantern-public-api.royzhao.dev`.
...
Requests to `https://lantern-api.royzhao.dev` and `https://lantern-public-api.royzhao.dev` terminate at Cloudflare's edge, travel through the outbound Cloudflare Tunnel connection maintained by `cloudflared`, then land on the app stack's local `nginx` boundary at `http://127.0.0.1:${NGINX_BIND_PORT}` for the matching environment.
...
5. Create the proxied DNS routes for both ingress hostnames:
   - `cloudflared tunnel route dns lantern-backend lantern-api.royzhao.dev`
   - `cloudflared tunnel route dns lantern-backend lantern-public-api.royzhao.dev`
...
7. From `ops/deployment/backend/tunnel/`, render and install the host-local config:
   - `./render-config.sh --tunnel-id "$TUNNEL_ID" --credentials-file "/etc/cloudflared/${TUNNEL_ID}.json" --output /tmp/cloudflared-config.yml`
...
2. Validate the public proof hostnames before Slice 4 origin protection is enabled:
   - `curl --fail --silent https://lantern-api.royzhao.dev/health/ready`
   - `curl --fail --silent https://lantern-public-api.royzhao.dev/health/ready`
